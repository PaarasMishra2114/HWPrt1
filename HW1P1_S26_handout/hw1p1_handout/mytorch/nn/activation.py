...
class Tanh:
    ...
    """
    def forward(self, Z):
        # Implement manually without using np.tanh()
        exp_Z = np.exp(Z)
        exp_neg_Z = np.exp(-Z)
...
class GELU:
    ...
    def backward(self, dLdA):
        cdf = 0.5 * (1 + scipy.special.erf(self.Z / np.sqrt(2)))
        pdf = (1 / np.sqrt(2 * np.pi)) * np.exp(-0.5 * self.Z ** 2)
        dAdZ = cdf + self.Z * pdf
        dLdZ = dLdA * dAdZ
        return dLdZ
...
class Swish:
    ...
    def backward(self, dLdA):
        dAdZ = self.sigmoid + self.Z * self.sigmoid * (1 - self.sigmoid) * self.beta
        dLdZ = dLdA * dAdZ
